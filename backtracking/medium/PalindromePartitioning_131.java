...
public class PalindromePartitioning_131 {
    ...
    // O(n^2*2^n) - time, O(n*2^n) - space (if we count result list as additional space)
    ...
    private void backtrack(String s, List<List<String>> partitions, List<String> part, int start) {
        if (start == s.length()) {
            partitions.add(new LinkedList(part)); //n
            return;
        }
    ...
    // O(n*2^n) - time, O(n^2*2^n) - space (if we count result list as additional space)
