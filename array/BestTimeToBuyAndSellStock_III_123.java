...
public class BestTimeToBuyAndSellStock_III_123 {
    ...
    public static void main(String[] args) {
        BestTimeToBuyAndSellStock_III_123 s = new BestTimeToBuyAndSellStock_III_123();
    ...
    public int maxProfit(int[] prices) {
        ...
        int[] maxProfitFirstPath = new int[prices.length];
        for (int i = 0; i < prices.length; i++) {
            minPriceSoFar = Math.min(minPriceSoFar, prices[i]);
            maxTotalProfit = Math.max(maxTotalProfit, prices[i] - minPriceSoFar);
            maxProfitFirstPath[i] = maxTotalProfit;
        }

        int maxPriceSoFar = Integer.MIN_VALUE;
        for (int i = prices.length-1; i > 0; i--) {
            maxPriceSoFar = Math.max(maxPriceSoFar, prices[i]);
            maxTotalProfit = Math.max(maxTotalProfit, maxPriceSoFar - prices[i] + maxProfitFirstPath[i-1]);
        }
