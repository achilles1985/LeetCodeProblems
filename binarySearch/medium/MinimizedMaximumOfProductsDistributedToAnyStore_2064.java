...
public class MinimizedMaximumOfProductsDistributedToAnyStore_2064 {
    ...
    public static void main(String[] args) {
        MinimizedMaximumOfProductsDistributedToAnyStore_2064 s = new MinimizedMaximumOfProductsDistributedToAnyStore_2064();
        System.out.println(s.minimizedMaximum(7, new int[]{15,10,10})); //5
        System.out.println(s.minimizedMaximum(6, new int[]{11,6})); //3
        System.out.println(s.minimizedMaximum(1, new int[]{100000})); //100000
    }
    ...
    public int minimizedMaximum(int n, int[] quantities) {
        ...
        while (left < right) {
            ...
            if (isPossible(mid, quantities, n)) { // when number of stores <= mid
                right = mid;
            } else {
                left = mid + 1;
            }
    ...
    boolean isPossible(int mid, int[] quantities, int stores) {
        int count = 0;
        for (int q: quantities) {
            count += q/mid;
            count += q%mid == 0 ? 0 : 1;
